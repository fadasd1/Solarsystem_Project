package logic_classes;

import classes.Vec;
import interfaces.Vector;
import interfaces.VectorMath;

/**
 * Singleton implementation of {@link VectorMath}.
 * Provides basic vector operations such as addition, subtraction,
 * scaling, dot product, unit vector calculation, and projection.
 */
public class VectorMathImpl implements VectorMath {

	/** Singleton instance */
	private static final VectorMath instance = new VectorMathImpl();

	/** Private constructor for singleton pattern */
	private VectorMathImpl() {}

	/** Returns the singleton instance */
	public static VectorMath getInstance() {
		return instance;
	}

	@Override
	public Vector addVectors(Vector v1, Vector v2) {
		return new Vec(v1.getX() + v2.getX(), v1.getY() + v2.getY(), v1.getZ() + v2.getZ());
	}

	@Override
	public Vector subtractVectors(Vector v1, Vector v2) {
		return new Vec(v1.getX() - v2.getX(), v1.getY() - v2.getY(), v1.getZ() - v2.getZ());
	}

	@Override
	public double magnitude(Vector v1) {
		return Math.hypot(v1.getX(), Math.hypot(v1.getY(), v1.getZ()));
	}

	@Override
	public double dotProductCoordinatesSafe(Vector v1, Vector v2) {
		double max1 = Math.max(Math.abs(v1.getX()), Math.max(Math.abs(v1.getY()), Math.abs(v1.getZ())));
		double max2 = Math.max(Math.abs(v2.getX()), Math.max(Math.abs(v2.getY()), Math.abs(v2.getZ())));
		if (max1 == 0 || max2 == 0) return 0;

		double x1 = v1.getX() / max1, y1 = v1.getY() / max1, z1 = v1.getZ() / max1;
		double x2 = v2.getX() / max2, y2 = v2.getY() / max2, z2 = v2.getZ() / max2;

		return (x1 * x2 + y1 * y2 + z1 * z2) * max1 * max2;
	}

	@Override
	public double dotProductCoordinates(Vector v1, Vector v2) {
		return v1.getX() * v2.getX() + v1.getY() * v2.getY() + v1.getZ() * v2.getZ();
	}

	@Override
	public Vector scale(Vector v, double lambda) {
		return new Vec(v.getX() * lambda, v.getY() * lambda, v.getZ() * lambda);
	}

	@Override
	public Vector unitVector(Vector v) throws ArithmeticException {
		double magnitude = magnitude(v);
		if (magnitude == 0) throw new ArithmeticException("Zero Magnitude");
		return new Vec(v.getX() / magnitude, v.getY() / magnitude, v.getZ() / magnitude);
	}

	@Override
	public double dotProductAngle(Vector v1, Vector v2, double angle) {
		return magnitude(v1) * magnitude(v2) * angle;
	}

	@Override
	public Vector componentAlphaToBeta(Vector alpha, Vector beta) throws ArithmeticException {
		double magnitudeSquared = magnitude(beta);
		if (magnitudeSquared == 0) throw new ArithmeticException("Zero Magnitude");
		magnitudeSquared *= magnitudeSquared;
		double lambda = dotProductCoordinates(alpha, beta) / magnitudeSquared;
		return scale(beta, lambda);
	}

	@Override
	public double angleBetween(Vector v1, Vector v2) {
		double cos = dotProductCoordinates(v1, v2) / (magnitude(v1) * magnitude(v2));
		cos = Math.max(-1.0, Math.min(1.0, cos));
		return Math.acos(cos);
	}

	@Override
	public Vector vectorAlphaToBeta(Vector alpha, Vector beta) {
		return subtractVectors(beta, alpha);
	}
}
